...
List<int> _collectYearSunriseSeconds(
  int year,
  Location location,
  double timezone,
) {
  ...
  for (var i = 0; i < days; i++) {
    ...
    out[i] = sunrise == null ? -1 : _secondsOfDay(sunrise);
...
void main() {
  final file = File('test/js_sunrise.json');
  if (!file.existsSync()) {
    stderr.writeln('missing test/js_sunrise.json');
    exitCode = 1;
    return;
  }
  ...
  final sunrise = data['sunrise'] as Map<String, dynamic>;
  final locationMap = sunrise['location'] as Map<String, dynamic>;
  ...
  final jsDays = (sunrise['days'] as List)
      .map((e) => (e as List).map((v) => (v as num).toInt()).toList())
      .toList();
  final dayCounts =
      (sunrise['dayCounts'] as List).map((v) => (v as num).toInt()).toList();
  ...
  for (var year = startYear; year <= endYear; year++) {
    ...
    final dartList = _collectYearSunriseSeconds(year, location, timezone);
