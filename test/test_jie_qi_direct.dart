...
import '../lib/src/sxwnl/solar_lunar_pos.dart'; // Add this import
...
void main() {
  ...
  // 尝试计算几个关键点
  // 2025春分 (0度)
  double jd_chunfen = qiAccurate(2 * pi * 25.0); 
  print('2025 Spring Equinox JD: ${jd_chunfen}');
  print('Time: ${AstroDateTime.fromJ2000(jd_chunfen)}');
  
  // Test 3: Test SSQ public methods
  print('\n--- Test 3: SSQ public methods (qiHigh, soHigh) ---');
  var ssq = SSQ();
  // 随便找个时间点验证方法可调用
  double jd_test = ssq.qiHigh(2 * pi * 25.0);
  print('SSQ.qiHigh(2025春分): ${AstroDateTime.fromJ2000(jd_test)}');
  
  double jd_so = ssq.soHigh(2 * pi * 123.0); // 随便一个合朔角度
  print('SSQ.soHigh(random): ${AstroDateTime.fromJ2000(jd_so)}');
