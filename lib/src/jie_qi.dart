...
// slot 0 = 1999 年春分 (黄经 0), slot 1 = 1999 年清明 (黄经 15度), ..., slot 24 = 2000 年春分。
// 对应的黄经弧度 w = slot * (π/12)。
//
...
int _currentSlot(double jd) {
  // 1. 粗放估算：以 1999年春分 (qiAccurate(0) ≈ JD -286.1) 为 slot 0 起点
  int slot = ((jd + 286.1) / 365.2422 * 24).floor();
  ...
  while (true) {
    ...
    // 1 秒的宽容度 (1/86400 天)。
    // AstroDateTime 内部会舍入到整数秒，roundTrip 回来的 jd 最多偏 0.5 秒。
    // 1 秒余量是最大误差的 2 倍，而相邻节气最短间隔约 14 天，完全安全。
    const epsilon = 1.0 / 86400;
