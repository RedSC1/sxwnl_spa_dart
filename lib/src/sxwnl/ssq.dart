...
class SSQ {
  ...
  /// ### 较高精度定气
  ///
  /// 主要用于生成日历排期，确保日期（哪一天）正确，而非获取精确的交节时刻。
  /// 若需获取极高精度的确切交节时间，请调用底层的 `qiAccurate`。
  ///
  /// **核心逻辑：**
  /// * **性能优先**：使用次高阶公式 `sALonT2` 估算，砍掉微小摄动项以提升速度。
  /// * **跨日拦截（防跳变）**：估算产生的几分钟误差通常不影响日期。但若结果落在午夜前后 20 分钟（1200秒）内，这几分钟的误差极易导致节气被排到错误的一天（如 23:59 变成 00:01）。此时会强制触发补偿，调用最高精度的 `sALonT` 重新计算。
  /// * **返回值**：已包含北京时间偏移的 J2000 相对天数。
  ...
  /// ### 较高精度定朔
  ///
  /// 主要用于推算农历大小月及初一的具体日期，而非获取精确的合朔时刻。
  /// 若需获取极高精度的确切合朔时间，请调用底层的 `soAccurate`。
  ///
  /// **核心逻辑：**
  /// * **性能优先**：使用次高阶公式 `msALonT2` 估算合朔时刻。
  /// * **跨日拦截（防跳变）**：因月球运动摄动更复杂，估算误差更大，跨日防御的拦截阈值扩大至午夜前后 30 分钟（1800秒）。触发后同样回退至最高阶公式重算。
  /// * **返回值**：已包含北京时间偏移的 J2000 相对天数。
  ...
  /// ### 低精度定朔
  ///
  /// 纯数学周期拟合的极速估算函数，完全脱离高精度星历引擎。
  ///
  /// **核心逻辑：**
  /// * **极速运算**：采用截断三角级数直接拟合，跳过所有查表和迭代。
  /// * **精度限制**：在公元 600-2000 年间，最大误差可达 2 小时。
  /// * **避坑指南**：严禁用于获取准确时间的场景。仅限对时间完全不敏感的粗略日历 UI 占位使用。
  ...
  /// ### 低精度定气
  ///
  /// 使用简化周期项直接拟合太阳视黄经的快速估算函数。
  ///
  /// **核心逻辑：**
  /// * **精度限制**：最大误差约 30 分钟。
  /// * **避坑指南**：同样脱离了高精度计算，仅推荐用于普通万年历场景占位。
  ...
  ///
  /// **核心逻辑与历史修正声明：**
  /// * **实历拟合**：本函数排出的并非纯粹的“现代天文理论农历”，而是**“中国历代真实农历”**。
  /// * **修正表接入**：内部获取节气和合朔时，统一调用了 `calc()` 方法。该方法不仅包含天文运算，更内置了**历代历法修正表**（尤其是明代及以前的平气/平朔历法时代）。它会模拟并还原古代天文官的“计算误差”，确保排出的历表、闰月与历史文献（实历）完全吻合。
  /// * **建正改历**：完整支持周历、新莽、魏明帝、武则天等特殊历史时期的建月与月名修正。
  ///
  /// [jd] 目标日期附近的 J2000 相对天数。
  /// 
  /// [enableHistoricalRules] 若为 true，则启用历史特殊规则（还原古历）；若为 false，则纯按现代规则强推理论历法。
