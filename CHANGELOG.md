...
- **修正时区感知日期相等性**：`AstroDateTime.operator ==` 与 `hashCode` 现在按实际表示的标准时刻比较，与 `compareTo()`、`difference()` 保持一致；同一时刻的不同民用时区表示可正确相等。

## 1.0.0 - 2026-07-31

- **首个稳定版本**：寿星万年历核心历法与天文扩展完成 Dart 公共 API 整理。
- **新增回历**：提供 `HuiLiDate` / `HijriDate` 算术回历及原版 30 年周期回归测试。
- **完善时区与精度**：提供不可变的 `AstroDateTime.toTimeZone()`，并保留 `withTimeZone()` 的纯标签语义；JD 反解保留可表示的秒以下精度。
- **补齐日月食与行星接口**：新增全球界线、地方日食南北界、均时差和行星低层事件辅助接口。
- **扩展冥王星事件支持**：现在 `xingLiu()` / `xingHR()` 已支持冥王星，使用已移植的 `pCoord()` 经度路径计算冥王星留与合/冲，补上原版 `XL0_calc(8, ...)` 越界缺口。
- **文档与验证**：补充算法架构说明、SPA/SXWNL 使用边界及 43 项全量测试回归。

- **完成寿星天文扩展移植**：新增 `xingXPosition()` / `xingX()` 太阳、月亮与水星至冥王星的站心星历链，补齐视黄道、视赤道、视差、地平坐标和标准折射输出。
- **新增恒星与星座模块**：机械提取并校验 `ephB.js` 的 `HXK` 恒星库、`xz88` 88 星座表及 IAU 2000B 章动表；移植 `getHXK()`、`schHXK()`、`hxCalcPositions()` 与恒星视位置修正链。
- **补齐寿星原始辅助接口**：月相照明、月球视半径、近远点、交点、地球近日点、`SZJ.calcRTS()` 多日升中降结果。
- **扩大公共导出**：包入口现在导出坐标转换、视差、大气折射、恒星时、章动和 ΔT 等底层寿星辅助函数，并提供原始大小写/下划线命名兼容别名。
- **验证**：新增原版 JavaScript 数值回归测试与生成表逐项校验脚本；XL0/XL1/Pluto、ephB 恒星/星座和 847 项章动系数均通过精确核对。

## 0.20.0

- **新增冥王星位置**：机械生成并纳入寿星原版 `XL0Pluto` 系数表，移植 `pluto_coord()` 和 P03 岁差转换；`pCoord(Planet.pluto, ...)` 现可返回冥王星黄经、黄纬和日心距离。
- **补充冥王星回归测试**：J2000 与 `t = 0.26` 位置结果均与原版 `p_coord(8, ...)` 对齐；后续 1.0.0 已补上冥王星 `xingLiu()` / `xingHR()` 快速事件接口。
